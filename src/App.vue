  ...
  <div class="min-h-screen bg-gray-100 p-4 md:p-8 print:p-0 print:bg-white font-sans text-gray-900">
          ...
          <div class="flex justify-between items-center border-b border-gray-200 pb-2 mb-4 print:mb-2 relative">
            ...
            <div class="print:hidden" v-if="savedClients.length > 0">
              <div class="flex items-center gap-1">
                <button @click="isClientDropdownOpen = !isClientDropdownOpen" type="button"
                  class="relative z-20 flex items-center justify-between w-40 bg-white border border-gray-300 rounded-lg px-3 py-1.5 text-sm text-gray-700 hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-gray-400 transition-all shadow-sm">
                  <span class="truncate font-medium">{{ selectedClientName || 'Kies klant...' }}</span>
                  <svg class="w-4 h-4 ml-1 opacity-70 transition-transform duration-200"
                    :class="{ 'rotate-180': isClientDropdownOpen }" fill="none" stroke="currentColor"
                    viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                  </svg>
                </button>
                <button @click="deleteClient"
                  class="text-red-400 hover:text-red-600 transition-colors disabled:opacity-30 p-1.5 rounded-lg hover:bg-red-50"
                  :disabled="!selectedClientName" title="Verwijderen">
              ...
              <div v-if="isClientDropdownOpen"
                class="absolute right-0 top-10 z-30 w-48 mt-1 bg-white rounded-xl shadow-xl border border-gray-100 overflow-hidden">
                <div class="max-h-48 overflow-y-auto py-1">
                  <button @click="selectCustomClient('')"
                    class="w-full text-left px-4 py-2 text-sm text-gray-500 hover:bg-gray-50 transition-colors italic">Kies
                    klant...</button>
                  <button v-for="c in savedClients" :key="c.name" @click="selectCustomClient(c.name)"
                    class="w-full text-left px-4 py-2 text-sm text-gray-800 hover:bg-gray-50 transition-colors font-medium">
                    {{ c.name }} </button>
                </div>
              </div>
            ...
            <tr v-for="item in invoiceData.items" :key="item.id"
              ...
              <td class="p-2 relative">
                <div class="relative w-full">
                  <input v-model="item.description" type="text" placeholder="Omschrijving"
                    ...
                    class="w-full border border-transparent hover:border-gray-300 focus:border-blue-500 rounded px-2 py-1 print:py-0 bg-transparent pr-8" />
                  <button @click="toggleItemDropdown(item.id)"
                    class="absolute right-1 top-1/2 -translate-y-1/2 text-gray-400 hover:text-blue-500 p-1 print:hidden"
                    title="Opgeslagen diensten">
                    <svg class="w-4 h-4 transition-transform duration-200"
                      :class="{ 'rotate-180': openItemDropdownId === item.id }" fill="none" stroke="currentColor"
                      viewBox="0 0 24 24">
                      <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path>
                    </svg>
                  </button>
                  <div v-if="openItemDropdownId === item.id"
                    class="fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-50 w-[90vw] max-w-sm bg-white rounded-xl shadow-2xl border border-gray-100 overflow-hidden md:absolute md:top-auto md:left-0 md:translate-x-0 md:translate-y-0 md:w-72 md:mt-1 md:shadow-xl md:border-gray-100 print:hidden">
                    <button @click.stop="saveCurrentItemAsTemplate(item)"
                      class="w-full text-left px-4 md:px-3 py-4 md:py-2 text-base md:text-sm text-blue-600 hover:bg-blue-50 border-b border-gray-100 transition-colors font-medium flex items-center gap-2">
                      <svg class="w-5 h-5 md:w-4 md:h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                          d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4" />
                      </svg> Opslaan als nieuw sjabloon </button>
                    <div class="max-h-[50vh] md:max-h-48 overflow-y-auto py-1">
                      <div v-for="si in savedItems" :key="si.description"
                        class="flex items-center justify-between px-3 md:px-2 py-3 md:py-1 hover:bg-blue-50 group transition-colors border-b border-gray-50 md:border-none">
                        <button @click="selectSavedItem(item, si)"
                          class="text-left text-base md:text-sm text-gray-800 font-medium truncate flex-1 px-2 py-1">{{
                            si.description }}</button>
                        <button @click.stop="deleteSavedItem(si.description)"
                          class="text-red-400 hover:text-red-600 p-2 md:p-1 rounded hover:bg-red-100 transition-colors"
                          title="Verwijderen">
                      ...
                      <div v-if="savedItems.length === 0"
                        class="px-4 py-4 text-base md:text-sm text-gray-500 italic text-center">Geen opgeslagen
                        diensten.</div>
                    </div>
                  </div>
                </div>
              ...
              <td class="p-2 text-center print:hidden">
                <button @click="removeItem(item.id)" class="text-red-400 hover:text-red-600 transition-colors font-bold"
                  title="Verwijderen" v-if="invoiceData.items.length > 1"> &times; </button>
              </td>
            </tr>
          </tbody>
        </table>
        <button @click="addItem"
          class="mt-4 text-blue-600 hover:text-blue-800 font-medium text-sm flex items-center gap-1 transition-colors print:hidden">
    ...
    <div class="mx-auto max-w-4xl mt-8 flex flex-col sm:flex-row justify-end gap-4 print:hidden">
      <button @click="fetchHistory"
        class="w-full sm:w-auto bg-gray-100 text-gray-700 border border-gray-300 hover:bg-gray-200 font-semibold py-3 px-8 rounded-xl shadow-sm transition-colors sm:mr-auto">
        Factuurgeschiedenis </button>
      <button @click="resetForm"
        class="w-full sm:w-auto bg-white text-gray-700 border border-gray-300 hover:bg-gray-50 font-semibold py-3 px-8 rounded-xl shadow-sm transition-colors">
        Nieuwe factuur </button>
      <button @click="openModal"
        class="w-full sm:w-auto bg-blue-600 hover:bg-blue-700 text-white font-semibold py-3 px-8 rounded-xl shadow-lg hover:shadow-xl transition-all transform hover:-translate-y-1">
